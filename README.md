...
| grace | `grace.start`, `grace.recover`, `grace.expire` | `payment_failed`, `grace_recover`, `grace_expire` | Grace при неоплате без движения денег: нулевые строки-маркеры, кэш прячется в held, исходы recover (кэш возвращён) / expire (подписка свёрнута, заряд не реверсится) | builder (sonnet) | 1 | [specs/10-grace.md](specs/10-grace.md) |
